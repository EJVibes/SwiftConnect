document.addEventListener("DOMContentLoaded", () => {
    // 1. Inject the warning banner first so it loads instantly
    injectFictionalWarning();
    
    // 2. Load the rest of the page components
    initMobileMenu();
...
function injectFictionalWarning() {
    ...
    // Prepends it as the very first element inside the body tag
...
function renderRouteList(routes, container) {
    ...
    filteredRoutes.forEach(r => {
        ...
        if (r.operator_name) {
            ...
            // Updated to clean URL structure
...
function buildTableHtml(records) {
    ...
    headers.forEach(h => {
        ...
        // Hides dynamically generated columns to keep headers clean
...
async function fetchAndRenderTimetable(routeId) {
    try {
        ...
        // Handles the independent tables generated by the Python scraper
